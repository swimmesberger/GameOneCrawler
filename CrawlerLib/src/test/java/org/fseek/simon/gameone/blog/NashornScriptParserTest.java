...
import org.fseek.simon.gameone.blog.video.NashornScriptParser;
import org.fseek.simon.gameone.blog.video.NashornScriptParser.SWFObject;
import org.fseek.simon.gameone.parse.ParseException;
import org.junit.After;
import org.junit.Before;
import org.junit.Test;

public class NashornScriptParserTest {
    ...
    // stuff like that is not allowed - security feature
    private static final String JAVASCRIPT_MALICIOUS_SAMPLE = "var File = Java.type(\"java.io.File\");";

    private NashornScriptParser scriptParser;

    @Before
    public void setUp() {
        this.scriptParser = new NashornScriptParser();
    }
    ...
    public void testVideoJavascript() throws ParseException {
        SWFObject evaluateJavascript = getScriptParser().parse(JAVASCRIPT_SAMPLE);
        assertEquals(9, evaluateJavascript.getVariables().size());
        assertEquals(5, evaluateJavascript.getParams().size());
        assertEquals("flash_container_632559_video_meta_632559", evaluateJavascript.getDestID());

        try {
            getScriptParser().parse(JAVASCRIPT_MALICIOUS_SAMPLE);
            throw new AssertionError("Security feature not working - access to Java File type.");
        } catch (ParseException ex) {
        }
    }

    public NashornScriptParser getScriptParser() {
        return scriptParser;
    }
}
